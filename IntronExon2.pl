...
###################################################################
# Generate Intron-Exon Postscript Figures from Genbank entries    #
# version 0.7                                                     #
# April 5, 2000                                               #
# Todd Richmond (todd@andrew2.stanford.edu)                       #
###################################################################

=head1 Generate Intron-Exon Postscript Figures from Genbank entries

=head1 Download script

The script is available at:

http://cellwall.stanford.edu/scripts/intronexon/IntronExon.pl

=head1 What does this script do?

Given a Genbank entry, or a series of Genbank entries, it will systematically
go through them, extract the CDS (coding sequences), output them to a file in
FASTA format, and draw a Postscript figure showing the intron-exon structure
of all of the coding sequences extracted. If the entries are annotated as 
described below, the script will also draw in the predicted transmembrane domains
for the genes.

=head1 What platforms does it work on?

It was originally designed for use on a Mac. It may very well run on
Windows or Unix, but I haven't tried.

=head1 How does it work?

Open IntronExon.pl in MacPerl and save as a Droplet. Drag and drop one or more 
Genbank entries on the script icon, and a prompt appears, asking for a root name.
 (If you're working under Windows or Unix, use the commandline line - "perl IntronExon.pl file1.dna file2.dna". 
 By default, two files are produced: a Postscript file of an intron-exon figure, 
 and a text file with all of the coding sequences from the entries in FASTA format. 
 If you give the root name "kinases", then the two files produced are named "kinases.ps" and 
 "kinases.fas". If logging is turned on internally in the script, then a third file is 
 produced "kinases.log" that contains some debugging information about what's happening. 
 The postscript figure produced is automatically scaled to fit on an 8.5 x 11 sheet of paper. 
 You can easily open the file in Adobe Illustrator and add some color, or other annotation if you wish.

=head1 Requirements

This script requires the use of three external packages:

=item B<Lincoln Stein's Boulder package for reading Genbank entries>

Information on this package is available at:

http://search.cpan.org/search?dist=Boulder

=item B<The BioPerl package for splicing together the coding sequences
and outputing the final sequences>
...
=item B<Shawn Wallace's Postscript package>

Information on this package is available at:

http://search.cpan.org/search?dist=PostScript

Unfortunately, Shawn's package wasn't complete enough for what
I wanted to do. I had to add an extra element in order to
incorporate text notation with lines, circles and boxes. So
in addition to Shawn's module, you'll need another file
that I've modified. Download it from:

http://cellwall.stanford.edu/scripts/figure/Elements2.pm
 
=head1 Special Features

If transmembrane domains are annotated in the DNA sequence, the
script will draw boxes in the final figure showing those features.
The annotation must be in a specific format as shown below:

     misc_feature    join(823..888,904..960,3416..3481,3497..3562,3602..3658,
                     3767..3832,3872..3928)
                     /gene="AtCslG1"
                     /note="transmembrane-region site"

the /gene field must match that of the corresponding CDS field:

     CDS             join(742..1326,2073..2222,2372..2500,2585..3028,
                     3137..4111)
                     /gene="AtCslG1"
                     /note="Arabidopsis cellulose synthase-like protein"
                     /codon_start=1
                     /product="AtCslG1"
                     
To annotate your genes properly, you can use the Sequin program from NCBI:

http://www.ncbi.nlm.nih.gov:80/Sequin/index.html

Annotate the transmembrane domains as a misc feature, and make sure the
comment/note field has the word "transmembrane" in it.

=cut

# these are core Perl modules
use File::Basename;
use File::Path;
use strict;

# Lincoln Stein's Boulder package
use Boulder::Genbank;
...
# This is a debugging log to help me trouble-shoot the drawing/positioning process
my $logging = "ON";
...
foreach my $file (@ARGV) 
{
	...
	foreach my $j (0 .. $#array) {
		...
		foreach my $i (0 .. $#misc_features) {
			...
			# if you remove the =~ m/transmembrane/ clause below, the script will draw all misc_features
			# this can get messy though...
			if (($seq_id =~ m/$gene_name/) and ($misc_features[$i]->Note =~ m/transmembrane/)) {
				...
				if ($feature_numbers =~ m/,/) { @feature_positions = split(/,/,$feature_numbers);}
				else { $feature_positions[0] = $feature_numbers;}
		...
		# we could use some improvement here - I'd like to be able to make it so long names
		# don't overlap the figure itself
		my $label_position;
		if ($label_font_size > $height) {
			# font size is bigger than feature - not a great idea, so make it level with the box
			$label_position = $y1;}
		else {
		...
		# now draw the features as a series of blue boxes slightly bigger (+10% on either side)
		# than the exon boxes
